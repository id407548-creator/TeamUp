...
const STORAGE_KEYS = {
    ...
    FILES: 'teamup_files', // 💡 누락되었던 파일 스토리지 키 추가
...
function getTeams() {
    ...
    const myMemberships = members.filter(m => Number(m.userId) === Number(session.userId));
    
    const myTeams = myMemberships.map(m => {
        const team = teams.find(t => Number(t.id) === Number(m.teamId));
        if (!team) return null;
        
        const memberCount = members.filter(mem => Number(mem.teamId) === Number(m.teamId)).length;
...
function getTeamDetails(teamId) {
    ...
    const membership = members.find(m => Number(m.teamId) === Number(teamId) && Number(m.userId) === Number(session.userId));
...
function getTeamMembers(teamId) {
    ...
    const teamMemberships = members.filter(m => Number(m.teamId) === Number(teamId));
    
    const memberList = teamMemberships.map(m => {
        const u = users.find(user => Number(user.id) === Number(m.userId));
        return {
            id: m.userId,
...
function inviteMemberToTeam(teamId, username) {
    const users = _load(STORAGE_KEYS.USERS);
    const targetUser = users.find(u => u.username === username.trim());
    if (!targetUser) return { success: false, message: '존재하지 않는 사용자 아이디입니다.' };

    const members = _load(STORAGE_KEYS.MEMBERS);
    const alreadyMember = members.find(m => Number(m.teamId) === Number(teamId) && Number(m.userId) === Number(targetUser.id));
...
function getTasks(teamId) {
    ...
    const teamTasks = tasks.filter(t => Number(t.teamId) === Number(teamId));
    const taskList = teamTasks.map(t => {
        const u = users.find(user => Number(user.id) === Number(t.assignedTo));
        return {
            ...t,
...
function deleteTask(taskId) {
    const tasks = _load(STORAGE_KEYS.TASKS);
    const filteredTasks = tasks.filter(t => Number(t.id) !== Number(taskId));
...
function getSchedules(teamId) {
    ...
    const teamSchedules = schedules.filter(s => Number(s.teamId) === Number(teamId));
...
function updateSchedule(scheduleId, title, description, startTime, endTime, color) {
    const schedules = _load(STORAGE_KEYS.SCHEDULES);
    const scheduleIndex = schedules.findIndex(s => Number(s.id) === Number(scheduleId));
...
// ==========================================
// E. 자료공유 가상 파일 관련 API (추가 보완)
// ==========================================
function getFiles(teamId) {
    const files = _load(STORAGE_KEYS.FILES);
    return { success: true, files: files.filter(f => Number(f.teamId) === Number(teamId)) };
}

function saveFile(teamId, fileName, fileSize, uploaderName) {
    const files = _load(STORAGE_KEYS.FILES);
    const now = new Date();
    ...
    _save(STORAGE_KEYS.FILES, files);
    return { success: true };
}

function deleteFile(fileId) {
    const files = _load(STORAGE_KEYS.FILES);
    const filtered = files.filter(f => Number(f.id) !== Number(fileId));
    _save(STORAGE_KEYS.FILES, filtered);
    return { success: true };
}

function deleteSchedule(scheduleId) {
    const schedules = _load(STORAGE_KEYS.SCHEDULES);
    const filteredSchedules = schedules.filter(s => Number(s.id) !== Number(scheduleId));
